...
async def change_email(user:UserUpdate, db:Session = Depends(get_db)):
...
async def change_password(user:UserUpdate, db:Session = Depends(get_db)):
    exist_user = db.execute(select(User).filter(User.id == user.id)).scalars().first()
    if not exist_user:
        raise HTTPException(status_code = 404, detail="User not found")
    # change password
...
async def login(loginModel: UserLogin, db:Session = Depends(get_db)):
    ...
    # check password
    hashed_password = hash_password(loginModel.password)
    if hashed_password == exist_user.hashed_password:
        return exist_user
    else:
